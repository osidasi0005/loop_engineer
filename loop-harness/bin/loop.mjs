...
function main() {
  ...
  while (state.iteration < cfg.loop.maxIterations) {
    ...
    saveSummary();
    console.log(
      `[${n}] エージェント ${agent.isError ? 'ERROR' : 'OK'} ${agent.ms}ms $${agent.costUsd.toFixed(4)} 累計$${state.totalCostUsd.toFixed(4)}${state.agentSaidDone ? ' 完了宣言あり（次の検証で確認）' : ''}`,
    );
